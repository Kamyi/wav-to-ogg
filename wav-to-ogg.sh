...
# line. This can be used as an alternative to log coloring, e.g. when dumping 
# the log to file. 
...
# path of records to convert. Modify only for optimization. For other purposes 
# modify $recordspath and $recordsdate
...
while IFS= read -r -d '' wavfile;do	
    #strings processing
    wavfilenopath="$(sed 's/.*\///' <<< "$wavfile")";
    oggfile="${wavfile//'.wav'/'.ogg'}"
    oggfilenopath="$(sed 's/.*\///' <<< "$oggfile" )";
    # sed is faster than basename
    #core convertation
    nice ffmpeg "$LOGLEVEL" "$LEVEL" -i "$wavfile" "$oggfile" \
      && rm -frv "$wavfile"
    
    if [ -f "$oggfile" ]; then
        mysql -u "$db_user" -p "$db_pass" -s -N -D "$database" \
          <<< "UPDATE cdr SET recordingfile='$oggfilenopath' \
                WHERE recordingfile = '$wavfilenopath'";fi

done < <(find "$corepath" -type f -name '*.wav')
